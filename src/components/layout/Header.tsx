...
const Header = () => {
  ...
  // Используем наш хук корзины
  const { calculateSummary } = useCart();
  ...
  const openCart = () => {
    setIsCartOpen(true);
    setIsMenuOpen(false); // Закрываем мобильное меню если открыто
  };

  return (
    <>
      <header className="sticky top-0 z-40 w-full backdrop-blur-lg bg-white/90 border-b border-emerald-100 shadow-sm">
        <div className="container mx-auto px-4">
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              {/* Favorites */}
              <Link href="/favorites">
                <Button variant="ghost" size="icon" className="hidden sm:flex relative border border-emerald-200 bg-white hover:bg-emerald-50">
                  <svg
              ...
              {/* Cart Button - ОБНОВЛЕНО */}
              <Button 
                variant="ghost" 
                size="icon" 
                className="relative border border-emerald-200 bg-white hover:bg-emerald-50 transition-all duration-200"
                onClick={openCart}
              >
                <svg
                  className="h-5 w-5 text-gray-600"
                ...
                {/* Cart Badge - Показываем количество товаров */}
                {cartSummary.itemCount > 0 && (
                  <Badge 
                    variant="default" 
                    className="absolute -top-2 -right-2 h-5 w-5 flex items-center justify-center p-0 text-xs bg-emerald-600 text-white animate-pulse"
                  >
                    {cartSummary.itemCount > 99 ? '99+' : cartSummary.itemCount}
                  </Badge>
                )}
                
                {/* Добавляем индикатор для новых товаров */}
                {cartSummary.itemCount > 0 && (
              ...
              {/* Profile */}
              <Link href="/login">
                <Button variant="outline" size="sm" className="hidden sm:flex text-gray-600 hover:text-emerald-600 hover:border-emerald-300">
                  Войти
                </Button>
              </Link>
              ...
              <div className="flex flex-col space-y-4">
                ...
                <div className="px-2 pt-2 border-t border-emerald-100 space-y-2">
                  ...
                  <button
                    ...
                    className="block w-full text-left py-2 text-gray-700 hover:text-emerald-600"
                  >
                    Корзина {cartSummary.itemCount > 0 && `(${cartSummary.itemCount})`}
                  </button>
